...
Input.C = 2; % bound for S, regulotry poentials
Input.M = 2; % bound for A, TF activities


%% NetREX
TotalEdge = [17];%TotalEdgeAll(KK);
KeepEdge = [10,11,13];
for ii = 1 : length(KeepEdge)
    for jj = 1 : length(TotalEdge)
        ...
        [Sbt] = NetREX_Bootstrap(Input, 5); % 5 is the bootstrap times
        Temp_bt.S = Sbt;
...
Cutoff = 15;
Predicted_Network = sparse(idr(Orval(1:Cutoff)), idc(Orval(1:Cutoff)), 1:length(Sval), Input.NumGene, Input.NumTF); % weight is the reanking, the lower the better
